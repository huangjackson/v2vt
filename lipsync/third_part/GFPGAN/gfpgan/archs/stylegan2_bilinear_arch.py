...
class EqualLinear(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bias=True, bias_init_val=0, lr_mul=1, activation=None):
        ...
        self.weight = nn.Parameter(torch.randn(
            out_channels, in_channels).div_(lr_mul))
        if bias:
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, x, style):
        ...
        if self.sample_mode == 'upsample':
            x = F.interpolate(
                x, scale_factor=2, mode=self.interpolation_mode, align_corners=self.align_corners)
        elif self.sample_mode == 'downsample':
            x = F.interpolate(
                x, scale_factor=0.5, mode=self.interpolation_mode, align_corners=self.align_corners)
...
class StyleGAN2GeneratorBilinear(nn.Module):
    ...
    def __init__(self,
        ...
        self.style_conv1 = StyleConv(
            ...
            interpolation_mode=interpolation_mode)
        self.to_rgb1 = ToRGB(
            channels['4'], num_style_feat, upsample=False, interpolation_mode=interpolation_mode)
