...
class GANimationModel(BaseModel):
    ...
    def setup(self):
        ...
        if self.is_train:
            ...
            # setup schedulers
            self.schedulers = [model_utils.get_scheduler(
                optim, self.opt) for optim in self.optims]

    def feed_batch(self, batch):
        self.src_img = batch['src_img'].to(self.device)
        self.tar_aus = batch['tar_aus'].type(torch.FloatTensor).to(self.device)
        if self.is_train:
            self.src_aus = batch['src_aus'].type(
                torch.FloatTensor).to(self.device)
            self.tar_img = batch['tar_img'].to(self.device)

    def forward(self):
        # generate fake image
        self.color_mask, self.aus_mask, self.embed = self.net_gen(
            self.src_img, self.tar_aus)
        self.fake_img = self.aus_mask * self.src_img + \
            (1 - self.aus_mask) * self.color_mask

        # reconstruct real image
        if self.is_train:
            self.rec_color_mask, self.rec_aus_mask, self.rec_embed = self.net_gen(
                self.fake_img, self.src_aus)
            self.rec_real_img = self.rec_aus_mask * self.fake_img + \
                (1 - self.rec_aus_mask) * self.rec_color_mask

    def backward_dis(self):
        # real image
        pred_real, self.pred_real_aus = self.net_dis(self.src_img)
        self.loss_dis_real = self.criterionGAN(pred_real, True)
        self.loss_dis_real_aus = self.criterionMSE(
            self.pred_real_aus, self.src_aus)
        ...
        if self.opt.gan_type == 'wgan-gp':
            self.loss_dis_gp = self.gradient_penalty(
                self.src_img, self.fake_img)
    ...
    def backward_gen(self):
        ...
        self.loss_gen_fake_aus = self.criterionMSE(
            self.pred_fake_aus, self.tar_aus)
        ...
        self.loss_gen = self.opt.lambda_dis * self.loss_gen_GAN \
            ...
            + self.opt.lambda_tv * \
            (self.loss_gen_smooth_real_aus + self.loss_gen_smooth_fake_aus)
    ...
    def get_latest_visuals(self):
        visuals_name = ['src_img', 'tar_img',
                        'color_mask', 'aus_mask', 'fake_img']
        if self.is_train:
            visuals_name.extend(
                ['rec_color_mask', 'rec_aus_mask', 'rec_real_img'])
