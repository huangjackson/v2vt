...
class BaseModel:
    ...
    def setup(self):
        ...
        if self.is_train:
            ...
            self.criterionGAN = model_utils.GANLoss(
                gan_type=self.opt.gan_type).to(self.device)
        ...
        for name in models_name:
            if isinstance(name, str):
                ...
                pretrained_state_dict = torch.load(
                    'checkpoints/30_net_gen.pth', map_location=str('cuda:0'))
                ...
                pretrained_dict = {
                    k: v for k, v in pretrained_state_dict.items() if k in net.state_dict()}
    ...
    def gradient_penalty(self, input_img, generate_img):
        ...
        inter_img = (alpha * input_img.data + (1 - alpha) *
                     generate_img.data).requires_grad_(True)
        ...
        dydx = torch.autograd.grad(outputs=inter_img_prob,
                                   ...
                                   grad_outputs=torch.ones(
                                       inter_img_prob.size()).to(self.device),
                                   retain_graph=True,
                                   create_graph=True,
                                   only_inputs=True)[0]
