    ...
    def process(self, img, ori_img, bbox=None, face_enhance=True, possion_blending=False):
        ...
        for i, (faceb, facial5points) in enumerate(zip(facebs, landms)):
            ...
            of, tfm_inv = warp_and_crop_face(
                img, facial5points, reference_pts=self.reference_5pts, crop_size=(self.size, self.size))
            ...
            mm = [0, 255, 255, 255, 255, 255, 255, 255,
            ...
            mask_sharp = cv2.warpAffine(
                mask_sharp, tfm_inv, (width, height), flags=3)
        ...
        mask_sharp = cv2.GaussianBlur(
            mask_sharp, (0, 0), sigmaX=1, sigmaY=1, borderType=cv2.BORDER_DEFAULT)
        ...
        if self.use_sr and img_sr is not None:
            ...
            if bbox is not None:
                ...
                full_img, ori_img, full_mask = [cv2.resize(
                    x, (512, 512)) for x in (full_img, ori_img, full_mask)]

            img = Laplacian_Pyramid_Blending_with_mask(
                full_img, ori_img, full_mask, 6)
