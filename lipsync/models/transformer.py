...
class Attention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        q = rearrange(self.to_q(x), 'b n (h d) -> b h n d',
                      h=self.heads)  # q,k from the zero feature
        k = rearrange(self.to_k(x), 'b n (h d) -> b h n d',
                      h=self.heads)  # v from the reference features
    ...
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                DualPreNorm(dim, Attention(dim, heads=heads,
