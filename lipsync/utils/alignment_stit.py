...
def paste_image(inverse_transform, img, orig_image):
    ...
    projected = img.convert('RGBA').transform(
        orig_image.size, Image.PERSPECTIVE, inverse_transform, Image.BILINEAR)
...
def align_face(filepath_or_image, predictor, output_size, detector=None,
    ...
    img = crop_image(filepath_or_image, output_size,
                     quad, enable_padding=enable_padding)
...
def crop_faces(IMAGE_SIZE, files, scale, center_sigma=0.0, xy_sigma=0.0, use_fa=False, fa=None):
    ...
    quads = np.stack([cs - xs - ys, cs - xs + ys, cs +
                     xs + ys, cs + xs - ys], axis=1)
    quads = list(quads)
